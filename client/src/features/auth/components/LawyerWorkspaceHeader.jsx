...
const navigationItems = [
  { label: 'Dashboard', path: '/lawyer/account', icon: LayoutDashboard },
  { label: 'Requests', path: '/lawyer/appointments', icon: Inbox },
  { label: 'Schedule', path: '/lawyer/schedule', icon: CalendarRange },
  { label: 'Profile', path: '/lawyer/profile', icon: BriefcaseBusiness },
];

const LawyerWorkspaceHeader = ({ activePath }) => (
  <header className="sticky top-0 z-40 border-b border-white/80 bg-white/75 backdrop-blur-xl">
    <div className="mx-auto flex max-w-7xl flex-wrap items-center justify-between gap-3 px-5 py-3 sm:px-8">
      <Link className="flex items-center gap-3" to="/lawyer/account">
      ...
      <nav className="order-3 flex w-full items-center gap-1 overflow-x-auto rounded-lg border border-white/90 bg-white/55 p-1 shadow-sm backdrop-blur-xl sm:order-none sm:w-auto" aria-label="Lawyer workspace">
...
export default LawyerWorkspaceHeader;
